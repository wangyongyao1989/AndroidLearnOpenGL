...
#include "../includes/GLSenior3DModel.h"

GLSenior3DModel::GLSenior3DModel(const string &path, bool gamma) {
    loadModel(path);
}

void GLSenior3DModel::Draw(GLSeniorShader &shader) {
    for (unsigned int i = 0; i < meshes.size(); i++)
        meshes[i].Draw(shader);
}


void GLSenior3DModel::loadModel(const string &path) {
...
void GLSenior3DModel::processNode(aiNode *node, const aiScene *scene) {
...
GLSenior3DMesh GLSenior3DModel::processMesh(aiMesh *mesh, const aiScene *scene) {
    // data to fill
    ...
    return GLSenior3DMesh(vertices, indices, textures);

}

vector <Texture>
GLSenior3DModel::loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName) {
    vector <Texture> textures;
...
unsigned int GLSenior3DModel::TextureFromFile(const char *path, const string &directory, bool gamma) {
