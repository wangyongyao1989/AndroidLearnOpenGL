...
bool GLSeniorCubeMap::setupGraphics(int w, int h) {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(CubeMapPlaneVertices), &CubeMapPlaneVertices, GL_STATIC_DRAW);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void *) 0);
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float),
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(CubeMapQuadVertices), &CubeMapQuadVertices, GL_STATIC_DRAW);
    glEnableVertexAttribArray(0);
...
bool
GLSeniorCubeMap::setSharderScreenPath(const char *vertexScreenPath, const char *fragmentScreenPath) {
    screenShader->getSharderPath(vertexScreenPath, fragmentScreenPath);
    return 0;
}

void GLSeniorCubeMap::setPicPath(const char *pic1, const char *pic2) {
