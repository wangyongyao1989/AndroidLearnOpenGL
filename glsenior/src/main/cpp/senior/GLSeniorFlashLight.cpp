...
#include <iostream>
#include "../includes/GLSeniorFlashLight.h"

bool GLSeniorFlashLight::setupGraphics(int w, int h) {
    screenW = w;
    screenH = h;
...
void GLSeniorFlashLight::renderFrame() {
...
bool GLSeniorFlashLight::setSharderPath(const char *vertexPath, const char *fragmentPath) {
    lightColorShader->getSharderPath(vertexPath, fragmentPath);
    return 0;
}

bool GLSeniorFlashLight::setColorSharderPath(const char *vertexPath, const char *fragmentPath) {
    lightCubeShader->getSharderPath(vertexPath, fragmentPath);
    return false;
}

void GLSeniorFlashLight::setPicPath(const char *pic1, const char *pic2) {
...
void GLSeniorFlashLight::setMoveXY(float dx, float dy, int actionMode) {
...
void GLSeniorFlashLight::setOnScale(float scaleFactor, float focusX, float focusY, int actionMode) {
...
GLSeniorFlashLight::GLSeniorFlashLight() {
...
void GLSeniorFlashLight::printGLString(const char *name, GLenum s) {
    const char *v = (const char *) glGetString(s);
    LOGI("OpenGL %s = %s\n", name, v);
}

void GLSeniorFlashLight::checkGlError(const char *op) {
 ...
 * @return
 */
int GLSeniorFlashLight::loadTexture(unsigned char *data, int width, int height, GLenum format) {
