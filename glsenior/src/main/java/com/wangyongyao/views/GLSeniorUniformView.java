...
public class GLSeniorUniformView extends GLSurfaceView implements GLSurfaceView.Renderer {

    private GestureDetector gestureDetector;
    private ScaleGestureDetector scaleGestureDetector;

    private static String TAG = GLSeniorUniformView.class.getSimpleName();
    ...
    public GLSeniorUniformView(Context context, GLSeniorCallJni jniCall) {
        super(context);
        mContext = context;
    ...
    public GLSeniorUniformView(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init();
    }
...
//                        + "===getFocusY" + detector.getFocusY());
                mGLSeniorCallJni.seniorUniformOnScale(scaleFactor, detector.getFocusX()
...
//                Log.e(TAG, "onScaleBegin: " + detector);
                mGLSeniorCallJni.seniorUniformOnScale(detector.getScaleFactor(), detector.getFocusX()
...
//                Log.e(TAG, "onScaleEnd: " + detector);
    ...
    public void onDrawFrame(GL10 gl) {
        if (mGLSeniorCallJni != null)
            mGLSeniorCallJni.seniorUniformOpenGLRenderFrame();
    }

    public void onSurfaceChanged(GL10 gl, int width, int height) {
        if (mGLSeniorCallJni != null)
            mGLSeniorCallJni.initSeniorUniformOpenGl(width, height);
    }
...
//                Log.e(TAG, "onTouchEvent: " + event.getAction());
                downX = event.getX();
                downY = event.getY();
                mGLSeniorCallJni.seniorUniformMoveXY(0, 0, 1);
            }
            break;
...
//                        + dx + "==dy:" + dy);
                mGLSeniorCallJni.seniorUniformMoveXY(dx, dy, 2);
            }
            break;
...
//                Log.e(TAG, "onTouchEvent: " + event.getAction());
                downX = 0;
                downY = 0;
                mGLSeniorCallJni.seniorUniformMoveXY(0, 0, 3);
            }
            break;
        }
