...
bool OpenglesDiffuseReflectionLight::setupGraphics(int w, int h) {
    screenW = w;
    screenH = h;
...
void OpenglesDiffuseReflectionLight::renderFrame() {
    ...
    model = glm::rotate(model, glm::radians(45.0f), diffuseReflectionLightPos);
...
bool OpenglesDiffuseReflectionLight::setSharderPath(const char *vertexPath, const char *fragmentPath) {
    lightColorShader->getSharderPath(vertexPath, fragmentPath);
    return 0;
}

bool OpenglesDiffuseReflectionLight::setColorSharderPath(const char *vertexPath, const char *fragmentPath) {
    lightCubeShader->getSharderPath(vertexPath, fragmentPath);
    return false;
}

void OpenglesDiffuseReflectionLight::setPicPath(const char *pic1, const char *pic2) {
...
void OpenglesDiffuseReflectionLight::setMoveXY(float dx, float dy, int actionMode) {
...
void OpenglesDiffuseReflectionLight::setOnScale(float scaleFactor, float focusX, float focusY, int actionMode) {
...
OpenglesDiffuseReflectionLight::OpenglesDiffuseReflectionLight() {
...
void OpenglesDiffuseReflectionLight::printGLString(const char *name, GLenum s) {
    const char *v = (const char *) glGetString(s);
    LOGI("OpenGL %s = %s\n", name, v);
}

void OpenglesDiffuseReflectionLight::checkGlError(const char *op) {
