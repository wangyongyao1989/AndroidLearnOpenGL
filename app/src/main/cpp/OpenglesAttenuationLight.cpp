...
bool OpenglesAttenuationLight::setupGraphics(int w, int h) {
    screenW = w;
    screenH = h;
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(AttenuationLightVertices), AttenuationLightVertices, GL_STATIC_DRAW);
...
void OpenglesAttenuationLight::renderFrame() {
...
//    lightColorShader->setVec3("light.position", AttenuationLightLightPos);
    ...
    model = glm::rotate(model, glm::radians(45.0f), AttenuationLightLightPos);
...
/*    // render the cube
    ...
    for (unsigned int i = 0; i < 10; i++)
    {
        ...
        glm::mat4 model = glm::mat4(1.0f);
        model = glm::translate(model, AttenuationLightCubePositions[i]);
        float angle = 20.0f * i;
...
bool OpenglesAttenuationLight::setColorSharderPath(const char *vertexPath, const char *fragmentPath) {
    lightCubeShader->getSharderPath(vertexPath, fragmentPath);
    return false;
}

void OpenglesAttenuationLight::setPicPath(const char *pic1, const char *pic2) {
...
void OpenglesAttenuationLight::setMoveXY(float dx, float dy, int actionMode) {
...
void OpenglesAttenuationLight::setOnScale(float scaleFactor, float focusX, float focusY, int actionMode) {
...
OpenglesAttenuationLight::OpenglesAttenuationLight() {
...
void OpenglesAttenuationLight::printGLString(const char *name, GLenum s) {
    const char *v = (const char *) glGetString(s);
    LOGI("OpenGL %s = %s\n", name, v);
}

void OpenglesAttenuationLight::checkGlError(const char *op) {
 ...
 * @return
 */
int OpenglesAttenuationLight::loadTexture(unsigned char *data, int width, int height, GLenum format) {
