...
bool OpenglesDirectionLight::setupGraphics(int w, int h) {
    screenW = w;
    screenH = h;
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(DirectionLightVertices), DirectionLightVertices, GL_STATIC_DRAW);
...
void OpenglesDirectionLight::renderFrame() {
...
//    depthTestShader->setVec3("light.position", DirectionLightLightPos);
    ...
    model = glm::rotate(model, glm::radians(45.0f), DirectionLightLightPos);
...
/*    // render the cube
 ...
 /*   // also draw the lamp object
    ...
    model = glm::mat4(1.0f);
    model = glm::translate(model, DirectionLightLightPos);
...
bool OpenglesDirectionLight::setColorSharderPath(const char *vertexPath, const char *fragmentPath) {
    lightCubeShader->getSharderPath(vertexPath, fragmentPath);
    return false;
}

void OpenglesDirectionLight::setPicPath(const char *pic1, const char *pic2) {
...
void OpenglesDirectionLight::setMoveXY(float dx, float dy, int actionMode) {
...
void OpenglesDirectionLight::setOnScale(float scaleFactor, float focusX, float focusY, int actionMode) {
...
OpenglesDirectionLight::OpenglesDirectionLight() {
...
void OpenglesDirectionLight::printGLString(const char *name, GLenum s) {
    const char *v = (const char *) glGetString(s);
    LOGI("OpenGL %s = %s\n", name, v);
}

void OpenglesDirectionLight::checkGlError(const char *op) {
 ...
 * @return
 */
int OpenglesDirectionLight::loadTexture(unsigned char *data, int width, int height, GLenum format) {
