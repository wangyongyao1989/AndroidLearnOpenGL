...
#include "GLSLFlashLight.h"
#include <iostream>

bool GLSLFlashLight::setupGraphics(int w, int h) {
    screenW = w;
    screenH = h;
...
void GLSLFlashLight::renderFrame() {
...
bool GLSLFlashLight::setSharderPath(const char *vertexPath, const char *fragmentPath) {
    lightColorShader->getSharderPath(vertexPath, fragmentPath);
    return 0;
}

bool GLSLFlashLight::setColorSharderPath(const char *vertexPath, const char *fragmentPath) {
    lightCubeShader->getSharderPath(vertexPath, fragmentPath);
    return false;
}

void GLSLFlashLight::setPicPath(const char *pic1, const char *pic2) {
...
void GLSLFlashLight::setMoveXY(float dx, float dy, int actionMode) {
...
void GLSLFlashLight::setOnScale(float scaleFactor, float focusX, float focusY, int actionMode) {
...
GLSLFlashLight::GLSLFlashLight() {
    lightColorShader = new GLSLShader();
    lightCubeShader = new GLSLShader();
}
...
void GLSLFlashLight::printGLString(const char *name, GLenum s) {
    const char *v = (const char *) glGetString(s);
    LOGI("OpenGL %s = %s\n", name, v);
}

void GLSLFlashLight::checkGlError(const char *op) {
 ...
 * @return
 */
int GLSLFlashLight::loadTexture(unsigned char *data, int width, int height, GLenum format) {
